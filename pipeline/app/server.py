...
back as a data URL. Request files exist only inside a temporary directory and
are deleted when the request finishes.
...
def _require_pipeline_key(provided: Optional[str]) -> None:
...
def pipeline_events(image_bytes: bytes, workdir: str) -> Generator[Dict[str, Any], None, None]:
    ...
    try:
        ...
        print(f"BOPB stage unavailable: {e}", flush=True)
        yield {"kind": "stage_done", "stage": "bopb", "t_ms": t(),
               "extra": {"duration_ms": int((time.monotonic() - s) * 1000),
                         "skipped": True, "error": "stage unavailable"}}
    ...
    try:
        ...
        print(f"CodeFormer stage unavailable: {e}", flush=True)
        yield {"kind": "stage_done", "stage": "codeformer", "t_ms": t(),
               "extra": {"duration_ms": int((time.monotonic() - s) * 1000),
                         "skipped": True, "error": "stage unavailable"}}
    ...
    try:
        ...
        print(f"ESRGAN stage unavailable: {e}", flush=True)
        yield {"kind": "stage_done", "stage": "esrgan", "t_ms": t(),
               "extra": {"duration_ms": int((time.monotonic() - s) * 1000),
                         "skipped": True, "error": "stage unavailable"}}
    ...
    try:
        ...
        print(f"AdaFace stage unavailable: {e}", flush=True)
        cosine, identity_warning, adaface_skipped = None, True, True
        yield {"kind": "stage_done", "stage": "adaface", "t_ms": t(),
               "extra": {"duration_ms": int((time.monotonic() - s) * 1000),
                         "skipped": True, "error": "stage unavailable",
    ...
    if input_is_gray:
        ...
        try:
            ...
            print(f"DDColor stage unavailable: {e}", flush=True)
            yield {"kind": "stage_done", "stage": "ddcolor", "t_ms": t(),
                   "extra": {"duration_ms": int((time.monotonic() - s) * 1000),
                             "skipped": True, "error": "stage unavailable"}}
    else:
    ...
    yield {"kind": "final", "t_ms": t(), "result": {
        ...
        "adaface_skipped": adaface_skipped,
    }}
...
def restore(
    ...
    x_pipeline_key: Optional[str] = Header(default=None),
):
    _require_pipeline_key(x_pipeline_key)
    try:
    ...
    try:
        ...
        return JSONResponse({"error": "restoration could not finish"}, status_code=500)
...
def restore_stream(
    ...
    def gen():
        try:
            ...
            print(f"restore stream failed: {e}", flush=True)
            yield json.dumps({
                "kind": "error",
                "t_ms": 0,
                "message": "restoration could not finish",
            }) + "\n"
